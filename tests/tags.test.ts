...
const plural: Card = { deck: 'Class', en: 'you come', pt: 'vocês vêm', tags: ['plural'] }
const object: Card = {
  deck: 'Class', en: 'him / it', pt: 'o', tags: ['masc', 'object'], ptTags: ['object'],
}
const sense: Card = { deck: 'Class', en: 'to be', pt: 'ser', sense: 'permanent / identity' }
const noun: Card = { deck: 'Class', en: 'friend', pt: 'o amigo', tags: ['masc'] }
...
describe('badgesFor', () => {
  ...
  it('renders nothing on the Portuguese face by default', () => {
    expect(badgesFor(plural, 'pt')).toEqual([])
    expect(badgesFor(noun, 'pt')).toEqual([])
  })

  // Except bare function words, which are ambiguous alone.
  it('renders ptTags on the Portuguese face', () => {
    expect(badgesFor(object, 'pt').map(b => b.pill)).toEqual(['OBJ'])
  })

  it('still renders the full tag set on the English face of a ptTags card', () => {
  ...
  it('sorts badges into canonical order regardless of source order', () => {
    const scrambled: Card = {
      deck: 'D', en: 'x', pt: 'y', tags: ['formal', 'plural', 'fem'],
    }
...
describe('hintFor', () => {
  ...
  it('does not repeat it on the Portuguese face', () => {
    expect(hintFor(sense, 'pt')).toBeUndefined()
  })
